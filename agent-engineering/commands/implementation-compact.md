...
Interactive entry point for compacting an active **implementation** phase. The full instruction set lives in this plugin's `sdd-flow` skill and is shared with the orchestrated flow — this command runs it in your own session instead of a spawned subagent.
...
The body is `SKILL_ROOT/bodies/implementation-compact.md`. If it cannot be found, say so and fall back to `/adhoc-compact` rather than improvising a template.
