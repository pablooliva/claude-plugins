...
Interactive entry point for compacting an active **research** phase. The full instruction set lives in this plugin's `sdd-flow` skill and is shared with the orchestrated flow — this command runs it in your own session instead of a spawned subagent.
...
The body is `SKILL_ROOT/bodies/research-compact.md`. If it cannot be found, say so and fall back to `/adhoc-compact` rather than improvising a template.
...
Report the compaction file path and where research stopped, then tell the user to `/clear` (or start a fresh session) and run `/continue`, which will find the compaction record and resume.
