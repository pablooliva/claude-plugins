...
Produce a proposed update as a unified diff or as explicit add/edit/remove instructions. For each new term, capture: canonical name, one-line definition, synonyms to avoid, and a code or research reference. Group terms by domain area (entities, actions, states, events, roles).
...
If `SDD/UBIQUITOUS_LANGUAGE.md` does not yet exist, create it. Apply the proposed update; resolve any contradictions explicitly (note the prior definition and why it changed).

If the research is purely infrastructural and introduces no domain vocabulary, conclude "no glossary changes" — record that in the progress file and skip the update.

## 4. Record Phase Transition

Append a brief transition note to `SDD/orchestration/progress.md` (append-only — do not overwrite existing content):

"Research phase complete. RESEARCH-[###]-[feature-name].md finalized. Glossary updated (or: no glossary changes). Ready for planning phase."

## Phase Complete

Return a summary of ≤200 words confirming phase completion. Include:
- Checklist verification status (all sections present, or list gaps)
- Glossary outcome (terms added/updated, or no changes)
- Paths to artifacts written or updated

Artifact paths to reference in your return:
- `SDD/research/RESEARCH-[###]-[feature-name].md`
