...
1. The `SPEC`, the `GLOSSARY`, and the `STANDARD` file.
...
- **Anything else under `SDD/`** — in particular `SDD/orchestration/progress.md`, `SDD/implementation/` (the IMPLEMENTATION-PLAN, `sites/SITES-IMPL-*`, slice files, retros, the ledger), `SDD/reviews/` (reviews, earlier `SITE-COUNT-*` and `SITE-DIFF-*` files — including your own earlier iterations), and `SDD/research/`.
...
For each in-scope control, count its sites **across the whole working tree** (committed and uncommitted code), not only inside the diff — the diff tells you which controls are in scope, not where their sites are.
...
3. For each path, find the place where the path commits to the rule — that is one **site**. Shared helpers are not sites; the call on each path is.
...
Follow `STANDARD` §4 column rules exactly — the orchestrator diffs your table mechanically. `Disposition`, `Evidence`, and `Slice` are always `—` in a blind count. Write `OUTPUT` once; it is immutable after writing.

### 5. Append to `progress.md` and return

`progress.md` is append-only; you may append to it but must not read it. Append exactly:

```markdown
## Site Count <SCOPE> iter <N> - Complete

- **Output:** <OUTPUT path>
- **Controls in scope:** <n>; **sites:** <n>; **gaps:** <n>
```
