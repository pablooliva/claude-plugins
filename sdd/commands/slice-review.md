...
You are conducting a per-slice code review for a single `SLICE-XXX` within a feature whose SPEC declares `delivery_mode: per-slice`. This command is a **thin wrapper over `/code-review`** (per OQ-5 conservative default — minimizes duplication; if slice-review behavior needs to diverge later, fork at that point, not preemptively).
...
2. **IMPLEMENTATION-PLAN's `## Slice Progress` row** at `In Progress` or `Acceptance Check Passing` — `/slice-review` reviews implemented code, so it expects a non-`Not Started`, non-`Complete` row.
3. **Error** — never silently pick. If priority 2 yields multiple candidates, prompt the user (supervised) or halt with an `## Awaiting Slicing Decision`-shaped block (autonomous).
...
Single match → use it. Multiple → prompt the user (supervised) or halt (autonomous). Zero → `No SPEC found in SDD/requirements/. Run /planning-start to create one before invoking /slice-review.`
...
- **`per-slice`** → continue with this command's normal behavior.
...
- `/slice-review` does NOT modify the table directly; `/slice-retro` updates `Status`/`Test result`/`Notes` after fix-and-rerun loops.

## Step 3: Resolve the active SLICE-XXX

Apply the **Active-Slice Resolution Convention** above. Expected statuses for `/slice-review`: `In Progress` or `Acceptance Check Passing`.
...
## Step 5: Apply `/code-review`'s logic, scoped to the slice's files

Read `sdd/commands/code-review.md` and apply its review process to the computed slice file set. The full review priority order applies:
...
**Apply Risk-Tiered Review Depth** per the slice's `Modules touched` field. For each MODULE-XXX in the slice's `Modules touched`, read the `Risk:` field from the SPEC's `## Modules` section and apply the matching depth (`high` = full internals review; `medium` = default; `low` = tested-boundary review only). See `sdd/commands/code-review.md` "Risk-Tiered Review Depth" section for the per-tier specifics. Implausible-tier override applies: if a module is marked `low` but you observe it touches state that is irreversible, security-sensitive, or financial, escalate.
...
The review document follows `/code-review`'s template (`SDD/reviews/REVIEW-XXX-...md` shape) with these slice-specific additions:
...
[All standard /code-review sections apply, scoped to the slice's REQ/EDGE/FAIL entries.]
...
[Standard /code-review decision rationale.]
```

## Per-Slice Review Iteration Cap (REQ-013, reference — enforced elsewhere)

`/slice-review` itself can be re-run any number of times. The **iteration cap of 3 with progress-stall check** (HIGH must strictly decrease across iterations; or MEDIUM when HIGH is zero) is enforced by the `sdd-flow` Step 4b/4c orchestration loop, NOT by this command. On halt, findings route to the rolling ledger's `Open recommendations awaiting user decision` section. In `--skip-slice-checkpoints` mode, the entire flow halts. See `agent-engineering/skills/sdd-flow/SKILL.md` Step 4b/4c for the loop logic.

## Flag Inventory (REQ-025 — applies to `/slice-review`)

`/slice-review` introduces NO flags of its own (per SPEC MODULE-002 + REQ-025: `/slice-review [SLICE-ID]` — no flags introduced by this feature). The `/slice-start` and `/slice-retro` flags do NOT apply here.
