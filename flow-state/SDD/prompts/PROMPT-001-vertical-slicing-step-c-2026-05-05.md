...
- [x] REQ-012: `sdd-flow` Step 4 mode-routes by `delivery_mode:` (whole-feature unchanged bit-for-bit; per-slice runs per-slice cycle per SLICE-XXX row) — Chunk 4b — Status: Complete (SKILL.md "Read delivery_mode and route" branch added at top of Step 4; per-slice cycle inserted before Session Resumption; whole-feature 4a–4j sub-step headers verified intact at lines 533–625; `## Delivery Slices` iteration documented; bit-for-bit preservation invariant satisfied)
- [x] REQ-013: Per-slice review iteration cap = 3 with progress-stall check; on halt route to ledger `Open recommendations awaiting user decision`; under `--skip-slice-checkpoints` halts whole flow — Chunk 4b — Status: Complete (per-slice 4c documents max 3 iterations, progress-stall check on HIGH (or MEDIUM when HIGH is zero), ledger routing `Open recommendations awaiting user decision`, halt-entire-flow under `--skip-slice-checkpoints`; recommendation matcher for `^## Recommended SPEC Amendments$` and `^## Recommended Re-planning$` documented in 4c.5)
- [x] REQ-014: Re-planning recommendation halts even under `--skip-slice-checkpoints`; resume options `/sdd-flow continue --replan [--from-slice SLICE-XXX]` and `--override-replan`; three-tier model surfacing — Chunk 4b — Status: Complete (per-slice 4c.5 emits the `Re-planning recommended.` halt block with the three resume options verbatim; halt-fires-regardless callout for autonomous + `--skip-slice-checkpoints`; EDGE-006 informative refusal for plain `/sdd-flow continue` while pending; Key Principle 16 added; flag inventory updated)
- [x] REQ-015: Slice subagents receive ONLY the rolling ledger in their prompts (per OQ-6) — Chunk 4b — Status: Complete (per-slice 4a explicitly states "**The subagent's prompt receives ONLY the ledger** (per OQ-6 default — strictly the ledger; individual retros are out of the prompt path)"; reinforced by Key Principle 15)
...
- [x] REQ-026: Cross-plugin version coupling README cross-references (sdd ↔ agent-engineering); FAIL-009 surfaced as warning — Chunk 5 — Status: Complete (sdd/README.md "Cross-plugin dependency" section names "SDD 2.0.0 requires agent-engineering 0.4.0 or later" and warns about FAIL-009 silent-misbehavior; agent-engineering/README.md "Requires SDD plugin 2.0.0 or later" subsection in Status section names FAIL-009 + recovery posture; both READMEs cross-reference each other; closing-oracle greps pass: `grep -c "agent-engineering" sdd/README.md` ≥1 and `grep -c "SDD 2.0" agent-engineering/README.md` = 2)
...
- **Chunk 4b (2026-05-05) — SKILL.md substantive (Step 4 per-slice state machine + checkpoint axis + re-planning + iteration cap).** Completed REQs: REQ-012 (full), REQ-013, REQ-014, REQ-015, REQ-024 (slice-boundary axis layer); EDGEs/FAILs closed: EDGE-006, FAIL-004. File modified: `agent-engineering/skills/sdd-flow/SKILL.md`. Surgery applied:
  - **Step 4 routing block** added at top of `### Step 4: Implementation Phase` ("Read delivery_mode and route") — names `whole-feature` (default, sub-steps 4a–4j unchanged), `per-slice` (branches to per-slice cycle), invalid (planning subagent already failed at 3a).
  - **Per-slice Step 4 cycle** new sub-section inserted after the whole-feature 4j and before `## Session Resumption`. Includes: slice-boundary checkpoint axis table (supervised/autonomous × slice-boundary on/off, default-on-in-both rationale); `Per-slice cycle (runs once per SLICE-XXX)` with steps 1 (per-slice 4a — implement; ledger-only prompt per OQ-6), 2 (per-slice 4b — slice-review), 3 (per-slice 4c — fix-and-re-review with max-3 cap, progress-stall check, ledger routing, autonomous-+-skip halt), 4 (per-slice 4c.5 — slice-retro with exact-string matcher for `^## Recommended SPEC Amendments$` and `^## Recommended Re-planning$`, three-tier resume options block for re-planning, halt-fires-regardless-of-skip-flag callout, EDGE-006 informative refusal), 5 (per-slice 4c.6 — atomic per-slice commit), 6 (slice-boundary PAUSE); `End-of-feature cycle` documents shared steps 7-13 (4d critical-review through 4j announcement).
  - **Arguments table** extended with six new flag rows: `--skip-slice-checkpoints`, `--fall-back-to-whole-feature`, `--retry-slicing "<hint>"`, `--replan`, `--from-slice SLICE-XXX`, `--override-replan` (lines 794–799).
  - **Key Principles** extended: principle 15 (per-slice cycle is strict — one subagent per slice, mandatory per-slice review, retro+ledger before commit, ledger-only prompts) and principle 16 (re-planning recommendations halt regardless of slice-boundary checkpoint setting).
  - **Per-Phase Sizing Strategy → Implementation (Step 4a)** clarified: REQ-count chunking heuristic applies to whole-feature mode only; per-slice mode uses one-subagent-per-slice strict rule.
  - **Closing-oracle gate passed:** all six grep blocks per Chunk 4b spec returned the expected hits — per-slice cycle headers (lines 528, 636, 649); both exact `^## Recommended *$` headers cited (lines 668, 669) with EDGE-006 narrative reference (line 679); slice-boundary axis (lines 640, 642–647, 663, 669, 677, 690, 784, 794); re-planning halt logic (lines 669, 677); per-slice review cap (line 657 — "max 3 iterations with progress-stall check"); all six flag rows (lines 794–799). Whole-feature 4a–4j sub-step headers verified intact (lines 533, 549, 557, 564, 572, 579, 587, 600, 621, 625) — bit-for-bit preservation invariant satisfied.
